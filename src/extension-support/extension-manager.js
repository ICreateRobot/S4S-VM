...
defaultBuiltinExtensions['ArduinoS4S'] = () => require('../extensions/ICreate_Arduino');
...
class ExtensionManager {
    ...
    // 新增移除扩展，根据id（未使用）
    // removeExtensionsByIds(extensionIds) {
    //     const idsToRemove = Array.isArray(extensionIds) ? extensionIds : [extensionIds];
        
    //     // 过滤出实际存在的扩展
    //     const existingIds = idsToRemove.filter(id => this.isExtensionLoaded(id));
    ...
    //     try {
    //         // 获取当前所有已加载扩展
    //         const loadedExts = Array.from(this._loadedExtensions.keys());
    //         console.log("当前所有已加载扩展",loadedExts)
            
    //         // 计算要保留的扩展
    //         const keepExts = loadedExts.filter(id => !existingIds.includes(id));
    //         console.log("计算要保留的扩展",keepExts)
            
    //         // 清空并重新加载
    //         this._reloadExtensions(keepExts);
            
    //         console.log(`已移除扩展: ${existingIds.join(', ')}，保留 ${keepExts.length} 个扩展`);
    //         return true;
            
    //     } catch (error) {
    //         console.error('批量移除扩展失败:', error);
    //         return false;
    //     }
    // }
    ...
    //  * 重新加载扩展列表
    //  */
    // _reloadExtensions(extensionIds) {
    //     // 清空当前状态
    //     this._loadedExtensions.clear();
        
    //     console.log("`重新加载扩展",extensionIds)
    //     // 重新加载扩展
    //     for (const id of extensionIds) {
    //         try {
    //             this.loadExtensionIdSync(id);
    //         } catch (e) {
    //             console.warn(`重新加载扩展失败 ${id}:`, e);
    //         }
    //     }
